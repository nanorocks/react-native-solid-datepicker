...
export const yearsGenerator = (min: number, max: number): {label: string, value: string}[] => {
  const years = [];
...
export const monthsGenerator = (): {label: string, value: string}[] => {
  return moment.monthsShort().map((monthName:string, index:number) => ({ label: monthName, value: (index + 1).toString() }));
};

export const daysGenerator: any = (month: string | null, year: string | null) : {label: string, value: string}[] => {

  try {
    const monthNum: number = parseInt(month)
    const yearNum: number = parseInt(year)

    const calculateMonth: string = monthNum > 9 ? monthNum.toString() : `0${monthNum.toString()}`;
